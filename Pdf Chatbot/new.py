...
def get_answer(docs: List[str], query: str, openai_api_key: str) -> str:
    """Gets an answer to a question using the GPT-3.5 Turbo model with document context."""
    openai.api_key = openai_api_key

    # Combine documents with the user query to form the prompt
    prompt = f'{docs}\nUser: {query}\nAI:'

    response = openai.Completion.create(
        engine="gpt-3.5-turbo",  # Use "text-davinci-003" engine for Chat models
        prompt=prompt,
        max_tokens=350  # You can adjust this parameter based on your desired response length
    )
...
text = """In the last chapter we developed the concepts of position,
...
a = text_split(text)
b = get_embeddings(text_split(text), openai_api_key="Your_Api_Key")
